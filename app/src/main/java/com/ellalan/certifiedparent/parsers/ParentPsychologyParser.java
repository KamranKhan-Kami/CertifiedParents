...
/**
 * Created by Kamran on 30-Sep-17.
 */

public class ParentPsychologyParser {
    Context context;

    public ParentPsychologyParser(Context context) {
    ...
    public String loadJSONFromAsset(Context context) {
        String json;
        try {
            InputStream is = context.getAssets().open("parentpsychology.json");
    ...
    public int getParentPsychologyStatementNumbers() {
        int n;
        try {
            JSONArray jsonArray = new JSONArray(loadJSONFromAsset(context));
            n = jsonArray.length();
