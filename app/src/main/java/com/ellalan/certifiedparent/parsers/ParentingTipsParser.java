...
public class ParentingTipsParser {
    Context context;

    public ParentingTipsParser(Context context) {
    ...
    public String loadJSONFromAsset(Context context) {
        String json;
        try {
            InputStream is = context.getAssets().open("parentingtips.json");
    ...
    public int getParentingTipsNumbers() {
        int n;
        try {
            JSONArray jsonArray = new JSONArray(loadJSONFromAsset(context));
            n = jsonArray.length();
